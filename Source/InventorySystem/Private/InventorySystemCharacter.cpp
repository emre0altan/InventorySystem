...
AInventorySystemCharacter::AInventorySystemCharacter()
{
	...
	HandMesh = CreateDefaultSubobject<USkeletalMeshComponent>(FName("Hand Mesh"));
	HandMesh->SetupAttachment(GetMesh());

	TorsoMesh = CreateDefaultSubobject<USkeletalMeshComponent>(FName("Torso Mesh"));
	TorsoMesh->SetupAttachment(GetMesh());

	LegsMesh = CreateDefaultSubobject<USkeletalMeshComponent>(FName("Legs Mesh"));
	LegsMesh->SetupAttachment(GetMesh());

	HairMesh = CreateDefaultSubobject<USkeletalMeshComponent>(FName("Hair Mesh"));
	HairMesh->SetupAttachment(GetMesh());

	BeardMesh = CreateDefaultSubobject<USkeletalMeshComponent>(FName("Beard Mesh"));
	BeardMesh->SetupAttachment(GetMesh());

	GlassesMesh = CreateDefaultSubobject<USkeletalMeshComponent>(FName("Glasses Mesh"));
	...
	SceneCaptureComponent = CreateDefaultSubobject<USceneCaptureComponent2D>(FName("Scene Capture 2D"));
...
void AInventorySystemCharacter::SetupPlayerInputComponent(class UInputComponent* PlayerInputComponent)
{
	...
	PlayerInputComponent->BindAction("PickupItem", IE_Pressed, this, &AInventorySystemCharacter::PickupItem);
}
...
void AInventorySystemCharacter::Tick(float delta) {
	Super::Tick(delta);
...
void AInventorySystemCharacter::OnOverlapEnd(UPrimitiveComponent* OverlappedComp, AActor* OtherActor, UPrimitiveComponent* OtherComp, int32 OtherBodyIndex) {
	...
	GEngine->AddOnScreenDebugMessage(-1, 5.f, FColor::Red, "ON OVERLAP END");
}

void AInventorySystemCharacter::PickupItem() {
	if (OverlappedActor && Cast<AItem>(OverlappedActor)) {
		Inventory->CollectItem(Cast<AItem>(OverlappedActor));
		OverlappedActor = nullptr;
	}
}
